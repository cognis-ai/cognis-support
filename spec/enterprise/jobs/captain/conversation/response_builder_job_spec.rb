...
RSpec.describe Captain::Conversation::ResponseBuilderJob, type: :job do
  ...
  describe '#perform' do
    ...
    context 'when captain_v2 is disabled' do
      ...
      it 'includes resolved activity markers when activity marker mode is enabled' do
        assistant.update!(config: { resolved_context_handling: 'activity_marker' })
        create(
          :message,
          conversation: conversation,
          message_type: :activity,
      ...
      it 'sends only messages after last_resolved_at when strict reset mode is enabled' do
        assistant.update!(config: { resolved_context_handling: 'strict_reset' })
