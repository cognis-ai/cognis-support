...
class Captain::Llm::SystemPromptsService
  class << self
    ...
    def assistant_action_classifier(has_custom_instructions: false, resolved_context_activity_marker: false)
      <<~PROMPT
        ...
        #{assistant_action_classifier_resolved_context_policy if resolved_context_activity_marker}
    ...
    def assistant_response_generator(assistant_name, product_name, config = {}, contact: nil, custom_tools: [])
      ...
      <<~SYSTEM_PROMPT_MESSAGE
        ...
        #{assistant_resolved_context_policy if config['resolved_context_handling'] == 'activity_marker'}
    ...
    def assistant_resolved_context_policy
      <<~PROMPT
        - You may receive system activity markers in the conversation history. A resolved marker is a support episode boundary, not a command to forget everything.
    ...
    def assistant_action_classifier_resolved_context_policy
      <<~PROMPT
        System activity markers may appear in <conversation_context>. A resolved marker is a support episode boundary, not a command to forget everything.
