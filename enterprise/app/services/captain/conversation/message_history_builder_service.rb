class Captain::Conversation::MessageHistoryBuilderService
  ...
  def conversation_messages_for_context
    ...
    messages = messages.where('created_at > ?', conversation.last_resolved_at) if strict_resolved_context_boundary?
    messages.where(message_type: context_message_types)
  end

  def strict_resolved_context_boundary?
    assistant.resolved_context_strict_reset? && conversation.last_resolved_at.present?
  end

  def context_message_types
    return [:incoming, :outgoing, :activity] if assistant.resolved_context_activity_marker?
